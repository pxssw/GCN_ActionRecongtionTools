...
class HRNetProcessor(object):
    ...
    def process(self):
        ...
        for video_id, video_path  in tqdm(enumerate(video_paths)):
            ...
            output_snippets_dir = '/6T-3/fengyu/output/snippets/{}'.format(video_path)

            tmp = int(video_path.split('C')[1].split('P')[0])
            if tmp == 2 or tmp == 3:
                output_sequence_dir = '/6T-3/fengyu/output/ntu_train'
            elif tmp == 1:
                output_sequence_dir = '/6T-3/fengyu/output/ntu_val'
            ...
            for idx, image_file in enumerate(sorted(image_files)):
                coco = {'people': []}
                json_name = os.path.join(output_snippets_dir, image_file.split('.')[0]+'.json')
                ...
                for pts in res_joint:
                    ...
                    coco['people'].append(person_dict)
                self.dump_coco_json(coco, json_name)
            video_info = self.json_pack(
                output_snippets_dir, video_path, width, height)
            ...
            video["index"] = int(video_path.split('A')[1]) - 1
            if tmp == 2 or tmp == 3:
        ...
        self.dump_coco_json(train_total_labels, train_output_label_path)
        self.dump_coco_json(val_total_labels, val_output_label_path)
